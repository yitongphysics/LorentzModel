...
string transitionFile = pathname + "transitionRate.txt";
string tetraFile = pathname + "100_0_tetra.txt";
string inputFile = pathname + "100_0.pos";
...
int main(int argc, char const *argv[]) {
    if (argc > 1){
        string epsilonStr       =   argv[1];
        string radiusRatioStr   =   argv[2];
        string stepStr          =   argv[3];
        string Dr0Str           =   argv[4];
        string TStr             =   argv[5];
        string tSkipStr         =   argv[6];
        string msdStr           =   argv[7];
        string inputStr         =   argv[8];

        stringstream epsilonSS(epsilonStr);
        stringstream radiusRatiosSS(radiusRatioStr);
        stringstream stepSS(stepStr);
        stringstream Dr0SS(Dr0Str);
        stringstream TSS(TStr);
        stringstream tSkipSS(tSkipStr);
        stringstream msdSS(msdStr);
    ...
    ofstream transitionfile(transitionFile, ios::out);
    ofstream posfile0(posFile, ios::out);
    
    // Compute the number of probes in each tetrahedron and print it out
    unordered_map<long int, int> probes_per_tetra;
    for(const auto& kv: tetraInfo){
        if(kv.second.is_percolation) probes_per_tetra[kv.first] = 0;
    }
    for (int ci = 0; ci < nC; ci++) {
        long int tid = probeTetra[ci];
        if (tid != -1) {
            probes_per_tetra[tid]++;
        }
    }
    for (const auto& kv : probes_per_tetra) {
        if(tetraInfo[kv.first].is_percolation){
            transitionfile << kv.first << " ";
        }
    }
    transitionfile << endl;
    for (const auto& kv : probes_per_tetra) {
        if(tetraInfo[kv.first].is_percolation){
            transitionfile << kv.second << " ";
        }
    }
    transitionfile << endl;
    ...
    for (long long t = 0; t < T; t++) {
        ...
        for (int ci = 0; ci < nC; ci++){
            ...
            if ((newTetra != probeTetra[ci]) & (t > tSkip)){
                ...
                transitionfile << " " << probeTetra[ci] << "," << newTetra;
            }
            probeTetra[ci] = newTetra;
        }
        if(ifTransition & (t > tSkip)) transitionfile << endl;
